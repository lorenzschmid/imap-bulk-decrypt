...
def decrypt(gpg, msg_str):
    ...
    if (d.ok or (d.status == 'signature valid' and d.valid)) and d.data != '':
        ...
        if NO_SECKEY in d.stderr:
            ...
            raise GpgError('Could not decrypt message string. GPG exited ' +
                           f'with "{d.status}".')
...
def translate(gpg, msg):
    if msg.is_multipart():
        # PGP/MIME
        if 'pgp-encrypted' in msg.get('Content-Type'):
        ...
        # Verify if one part of multipart message is encrypted
            ...
            raise MultipartEncrypted(
                'Multipart Message is not of type PGP/MIME and thus not ' +
                'supported.')
