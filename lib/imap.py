...
class Imap:
    ...
    def _search_folder(self):
        ...
        ok, res = self.imap.search(
            None, '(HEADER Content-Type "pgp-encrypted")')
        if ok == 'OK':
    ...
    def get_msg(self, uid):
        ...
        if ok != 'OK':
            raise InvalidMsgId('Could not fetch flags for message {}.'.format(
                uid))
        elif response:
    ...
    def cp_msg(self, uid, path):
        ...
        if ok != 'OK':
            raise OperationFailed(
                'Could not copy message {} to {}.'.format(uid, path))
